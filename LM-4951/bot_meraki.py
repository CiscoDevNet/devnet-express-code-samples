...
def do_meraki(resp):
    ...
    if trig[1].lower() in ['organization', 'organizations', 'org', 'orgs']:
        keyword = 'org'
        endpoint = '<put organization api endpoint here>'
    elif trig[1].lower() in ['networks', 'network', 'net']:
        if len(trig) > 2:
            keyword = 'net'
            endpoint = '/organizations/' + trig[2] + '<put network api endpoint here>'
        else:
        ...
        if len(trig) > 2:
            keyword = 'dev'
            endpoint = '/networks/' + trig[2].upper() + '<put devices api endpoint here>'  # /networks/[networkId]/devices
            log.info(endpoint)
        else:
...
def add_user(org, user, header):
    payload = {"name": "<put bot name here>", "email": "<put bot email here>", "orgAccess": "full", "tags": []}
