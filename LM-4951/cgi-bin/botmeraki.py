...
meraki_key = "PUT MERAKI API KEY HERE" #+ "!" #comment to enable Meraki API
...
def do_meraki(resp):
    ...
    if trig[1].lower() in ['organization', 'organizations', 'org', 'orgs']:
        keyword = 'org'
        endpoint = '<PUT MERAKI API FOR ORGANIZATIONS HERE>'
    elif trig[1].lower() in ['networks', 'network', 'net']:
        if len(trig) > 2:
            keyword = 'net'
            endpoint = '/organizations/' + trig[2] + '<PUT MERAKI API FOR NETWORKS HERE>''
        else:
        ...
        if len(trig) > 2:
            keyword = 'dev'
            endpoint = '/networks/' + trig[2].upper() + '<PUT MERAKI API FOR DEVICES HERE>'  # /networks/[networkId]/devices
            log.info(endpoint)
        else:
        ...
        if len(trig) > 2:
            keyword = 'sw'
            endpoint = '/devices/' + trig[2].upper() + '<PUT MERAKI API FOR SWITCHPORTS HERE>'  # /devices/[serial]/switchPorts
            log.info(endpoint)
        else:
            Spark.post_markdown(accessToken, resp['roomId'],
        ...
        if len(trig) > 2:
            keyword = 'cli'
            endpoint = '/devices/' + trig[2].upper() + '<PUT MERAKI API FOR CLIENTS HERE>?timespan=86400'  # /devices/[serial]/switchPorts
            log.info(endpoint)
        else:
            Spark.post_markdown(accessToken, resp['roomId'],
        ...
        if len(trig) > 2:
                keyword = 'admin'
                endpoint = "/organizations/" + trig[2].upper() + "<PUT MERAKI API FOR ADMINS HERE>"
        else:
...
if __name__ == '__main__':
    log_file()
    accessToken = '<PUT BOT ACCESS TOKEN HERE>'
