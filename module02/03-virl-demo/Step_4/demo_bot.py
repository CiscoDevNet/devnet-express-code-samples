...
def ngrok():
    if platform.system() == "Windows":
        ...
        try:
            ...
            if "x" in check_ngrok:
                print(
                    "ngrok was successfully downloaded and moved to PATH folder")
                return True
            else:
    ...
    if check_webhook.ok:
        ...
        if len(check_webhook["items"]) > 0:
            for items in check_webhook["items"]:
                webhooks[items["resource"]] = items["id"]
    if len(webhooks) == 0:
        for resource in resources:
            payload = {"name": name, "targetUrl": target_url,
                       "resource": resource, "event": "created"}
        ...
        for resource in resources:
            payload = {"name": name, "targetUrl": target_url}
            webhook = send_spark_put(
                url + "/" + webhooks[resource], payload, js=False)
            if webhook.ok:
                status = True
                print("Webhook was successfully updated")
            else:
                status = False
                print(
                    "Something went wrong. I was unable to update the webhook")
